...
if __name__ == "__main__":
    ...
    print(".......")
    results = {}
    model_name = args.image_folder.split('/')[-1]
    if model_name == '':
        model_name = args.image_folder.split('/')[-2]
    ...
    for item in files:
        image_path = os.path.join(args.image_folder, item)
        images = load_data(image_path)
        mean, std = get_inception_score(images)
        # mean,std = 0, 0
        results[item] = [mean, std]
        print(item, mean, std)
        # break

    for k,v in results.items():
        print("{}: mean: {:.2f} std: {:.2f}".format(k, v[0], v[1]))
    # save results to file
    file_name = os.path.join('/ACTesting/results/IS_'+ model_name + '.txt')
